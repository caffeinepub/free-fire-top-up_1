...
export default function App() {
  ...
  // Wait up to 30 seconds for actor to be ready
  const waitForActor =
  ...
  const handleManualSubmit = useCallback(async () => {
    ...
    if (backendSuccess) {
      ...
      // Backend failed after 30s wait — show an error so admin can be notified
      if (statusIntervalRef.current) clearInterval(statusIntervalRef.current);
      setIsLoading(false);
      setError(
        "Could not connect to server after 30 seconds. Please check your connection and try again. Your order was NOT saved.",
      );
      return;
    }
    ...
    return (
      <div className="px-4 py-4 pb-2">
        ...
        <a
          ...
          className="flex items-center gap-3 px-4 py-3.5 rounded-2xl mb-4 transition-all duration-200 hover:brightness-110 active:scale-[0.99]"
          style={{
            background: "#0d3320",
            border: "1px solid rgba(74,222,128,0.3)",
          }}
          data-ocid="whatsapp.contact_banner"
        >
          <div
  ...
  return (
    ...
    >
      ...
      <a
        ...
        className="fixed bottom-20 right-4 z-40 flex items-center gap-2 px-3 py-2.5 rounded-full shadow-lg transition-all duration-200 hover:scale-105 active:scale-95 group"
        style={{
          background: "#0d3320",
